...
import { MessageCircle, ClipboardList, User, LogOut, ChevronLeft } from "lucide-react";
import { useAuth } from "@/hooks/useAuth";
import N8nAutoTrigger from "@/components/N8nAutoTrigger";

const navItems = [
  { to: "/delivery", icon: MessageCircle, label: "Conversas", end: true },
  { to: "/delivery/pedidos", icon: ClipboardList, label: "Pedidos" },
  { to: "/delivery/perfil", icon: User, label: "Perfil" },
];
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <header className="sticky top-0 z-30 bg-admin-nav/95 backdrop-blur-md border-b border-admin-card-border px-4 h-14 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate("/delivery/perfil")} className="text-muted-foreground">
            <ChevronLeft size={20} />
          </button>
          <span className="text-sm text-muted-foreground">Delivery</span>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-sm text-foreground">{profile?.name || "Entregador"}</span>
