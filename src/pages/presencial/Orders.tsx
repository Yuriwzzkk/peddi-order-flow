...
const statusFlow: Record<string, { label: string; color: string; next?: string; action?: string }> = {
  ...
  ready: { label: "Pronto", color: "bg-status-ready", next: "completed", action: "Finalizar" },
...
export default function PresencialOrders() {
  ...
  useEffect(() => {
    ...
    listOrders(restaurantId)
      .then((data) => setOrders(data.filter((o) => o.type === "presencial")))
  ...
  useEffect(() => {
    ...
    const sub = subscribeOrders(restaurantId, (order, event) => {
      if (order.type !== "presencial") return;
      setOrders((prev) => {
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-xl font-bold text-foreground">Pedidos do dia</h1>

      <div className="flex gap-2 overflow-x-auto pb-2 -mx-4 px-4">
        {["all", "new", "confirmed", "preparing", "ready", "completed"].map((s) => (
          <button
            key={s}
            onClick={() => setFilter(s)}
              ...
              <div className="p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`text-[10px] px-2 py-0.5 rounded-full text-white ${statusFlow[order.status].color}`}
                    >
                ...
                {statusFlow[order.status].next && (
                  <Button
                    ...
                    className="w-full h-11 bg-primary hover:bg-primary-hover text-primary-foreground text-sm font-semibold rounded-xl"
                  >
                    <Check size={16} className="mr-1" /> {statusFlow[order.status].action}
                  </Button>
                )}
              </div>
            </motion.div>
